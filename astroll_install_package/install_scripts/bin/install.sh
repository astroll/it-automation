...
############################################################
func_release_place() {
	if [ ${1} = "astroll_ita_base" ]; then
		DRIVER=`echo ${1} | cut -d "_" -f 3 | sed 's/^ \(.*\) $/\1/'`
	else
...
RELEASE_PLASE=(
    astroll_ansible-driver
    astroll_cobbler-driver
    astroll_openstack-driver
    astroll_dsc-driver
    astroll_material
    astroll_material2
    astroll_material3
    ...
    if [ "$LINE" ]; then
        ...
        if [ "$key" = 'ita_base' -a "$val" = 'no' ]; then
            ...
            if ! test -d "$ITA_DIRECTORY"/ita-root ; then
                log 'ERROR : It is necessary to install astroll main functions (ita_base).'
                log 'INFO : Abort installation.'
                func_exit_and_delete_file
            fi
            ...
            if [ "$val" = 'yes' ]; then
                if test -d "$ITA_DIRECTORY"/ita-root ; then
                    log 'ERROR : astroll main functions (ita_base) have already been installed.'
                    log 'INFO : Abort installation.'
                    func_exit_and_delete_file
                fi
            fi
...
# /tmp/astroll_answers.txtを削除
rm -f "$COPY_ANSWER_FILE" 2>> "$LOG_FILE"

# astroll本体、ドライバがすべてnoの場合は処理を終了
...
#ドライバがインストールされているか確認
if [ "$ANSIBLE_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_ansible-driver ; then
...
if [ "$COBBLER_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_cobbler-driver ; then
...
if [ "$OPENSTACK_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_openstack-driver ; then
...
if [ "$DSC_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_dsc-driver ; then
...
if [ "$MATERIAL_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_material ; then
        log 'WARNING : Material has already been installed.'
        MATERIAL_FLG=0
    fi
fi

if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_material2 ; then
    MATERIAL2_FLG=0
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_ansible-driver" ] && [ "$MATERIAL_FLG" -eq 1 ] ; then
    MATERIAL2_FLG=1
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_material" ] && [ "$ANSIBLE_FLG" -eq 1 ] ; then
    MATERIAL2_FLG=1
elif [ "$ANSIBLE_FLG" -eq 1 ] && [ "$MATERIAL_FLG" -eq 1 ] ; then
    MATERIAL2_FLG=1
fi

if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_material3 ; then
    MATERIAL3_FLG=0
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_openstack-driver" ] && [ "$MATERIAL_FLG" -eq 1 ] ; then
    MATERIAL3_FLG=1
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_material" ] && [ "$OPENSTACK_FLG" -eq 1 ] ; then
    MATERIAL3_FLG=1
elif [ "$OPENSTACK_FLG" -eq 1 ] && [ "$MATERIAL_FLG" -eq 1 ] ; then
    MATERIAL3_FLG=1
fi

if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_material5 ; then
    MATERIAL5_FLG=0
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_dsc-driver" ] && [ "$MATERIAL_FLG" -eq 1 ] ; then
    MATERIAL5_FLG=1
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_material" ] && [ "$DSC_FLG" -eq 1 ] ; then
    MATERIAL5_FLG=1
...
if [ "$CREATEPARAM_FLG" -eq 1 ]; then
    if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_createparam ; then
...
if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_hostgroup3 ; then
    HOSTGROUP3_FLG=0
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_ansible-driver" ] && [ "$HOSTGROUP_FLG" -eq 1 ] ; then
    HOSTGROUP3_FLG=1
elif [ -e "$ITA_DIRECTORY/ita-root/libs/release/astroll_hostgroup" ] && [ "$ANSIBLE_FLG" -eq 1 ] ; then
    HOSTGROUP3_FLG=1
elif [ "$ANSIBLE_FLG" -eq 1 ] && [ "$HOSTGROUP_FLG" -eq 1 ] ; then
    HOSTGROUP3_FLG=1
fi

if test -e "$ITA_DIRECTORY"/ita-root/libs/release/astroll_hostgroup2 ; then
    HOSTGROUP2_FLG=0
...
if [ "$BASE_FLG" -eq 1 ]; then
    ...
    #################################################################################################
    log "INFO : `printf %02d $PROCCESS_CNT`/$PROCCESS_TOTAL_CNT Create database and user for astroll."
    #################################################################################################
    ...
    #################################################################################################
    log "INFO : `printf %02d $PROCCESS_CNT`/$PROCCESS_TOTAL_CNT Write astroll-it-automation in /etc/hosts."
    #################################################################################################
    echo '127.0.0.1    astroll-it-automation' >> /etc/hosts 2>> "$LOG_FILE"
    PROCCESS_CNT=$((PROCCESS_CNT+1))
    ...
    #################################################################################################
    if [ ${ITA_OS} = 'RHEL6' ]; then
        cp -p ../ext_files_for_CentOS6.x/etc_pki_tls_certs/astroll-it-automation.crt /etc/pki/tls/certs/ 2>> "$LOG_FILE"
    else
        cp -p ../ext_files_for_CentOS7.x/etc_pki_tls_certs/astroll-it-automation.crt /etc/pki/tls/certs/ 2>> "$LOG_FILE"
    fi
    if ! test -e /etc/pki/tls/certs/astroll-it-automation.crt ; then
        log 'WARNING : Failed to place /etc/pki/tls/certs/astroll-it-automation.crt.'
    fi

    if [ ${ITA_OS} = 'RHEL6' ]; then
        cp -p ../ext_files_for_CentOS6.x/etc_pki_tls_certs/astroll-it-automation.key /etc/pki/tls/certs/ 2>> "$LOG_FILE"
    else
        cp -p ../ext_files_for_CentOS7.x/etc_pki_tls_certs/astroll-it-automation.key /etc/pki/tls/certs/ 2>> "$LOG_FILE"
    fi

    if ! test -e /etc/pki/tls/certs/astroll-it-automation.key ; then
        log 'WARNING : Failed to place /etc/pki/tls/certs/astroll-it-automation.key.'
    fi
    PROCCESS_CNT=$((PROCCESS_CNT+1))
    ...
    #################################################################################################
    if [ ${ITA_OS} = 'RHEL6' ]; then
        cp -p ../ext_files_for_CentOS6.x/etc_httpd_conf.d/vhosts_astroll-it-automation.conf /etc/httpd/conf.d/ 2>> "$LOG_FILE"
    else
        cp -p ../ext_files_for_CentOS7.x/etc_httpd_conf.d/vhosts_astroll-it-automation.conf /etc/httpd/conf.d/ 2>> "$LOG_FILE"
    fi
    if test -e /etc/httpd/conf.d/vhosts_astroll-it-automation.conf ; then
        sed -i -e "s:$REPLACE_CHAR:$ITA_DIRECTORY:g" /etc/httpd/conf.d/vhosts_astroll-it-automation.conf 2>> "$LOG_FILE"
    else
        log 'WARNING : Failed to place /etc/httpd/conf.d/vhosts_astroll-it-automation.conf.'
    fi
    ...
    #################################################################################################
    log "INFO : `printf %02d $PROCCESS_CNT`/$PROCCESS_TOTAL_CNT Create directory to place astroll."
    #################################################################################################
    ...
    #################################################################################################
    log "INFO : `printf %02d $PROCCESS_CNT`/$PROCCESS_TOTAL_CNT Place astroll full functions."
    #################################################################################################
    cp -rp ../ITA/ita-contents/ita-root "$ITA_DIRECTORY"/ 2>> "$LOG_FILE"
    if ! test -d "$ITA_DIRECTORY"/ita-root ; then
        log 'WARNING : Failed to place astroll full functions.'
    fi
    ...
    ##################################################################################################
    #Place astroll release file for base functions."
    ##################################################################################################
    func_release_place astroll_ita_base
    
    ################################################################################################
    #Place astroll config files for base functions."
    ################################################################################################
    ...
    #################################################################################################
    log "INFO : `printf %02d $PROCCESS_CNT`/$PROCCESS_TOTAL_CNT Create symbolic link of astroll environment file."
    #################################################################################################
    ...
    ################################################################################################
    #Set up astroll services for base functions."
    ################################################################################################
    if [ ${ITA_OS} = 'RHEL6' ]; then
    ...
    #################################################################################################
    #Set up astroll crontab for base functions."
    #################################################################################################
