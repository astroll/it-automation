...
configure_cobbler() {
    ...
    #Replace with astroll file(tftp)
    ...
    #Replace with astroll file(debmirror.conf)
    copy_and_backup "$ITA_EXT_FILE_DIR/etc/debmirror.conf" "/etc/"
    
    #Replace with astroll file(pxedefault.template)
...
configure_ita() {
    ...
    # install astroll
    "$ITA_INSTALL_SCRIPTS_DIR/astroll_installer.sh"
}
...
make_ita() {
    ...
    log "Running the astrollinstaller"
    configure_ita
}
...
download() {
    ...
    ITA_VERSION=`cat $ITA_INSTALL_PACKAGE_DIR/ITA/ita-releasefiles/astroll_ita_base | cut -f 7 -d " "`
    DATE=`date +"%Y%m%d%H%M%S"`
	
	OFFLINE_INSTALL_FILE="astroll_Ver"$ITA_VERSION"_offline_"$DATE".tar.gz"
    ...
    (
        if [ ! -e "ITA_PACKAGE_OPEN_DIR/$OFFLINE_INSTALL_FILE" ]; then
            ...
            tar zcf $OFFLINE_INSTALL_FILE astroll_install_package >> "$ITA_BUILDER_LOG_FILE" 2>&1
        else
...
ITA_ANSWER_FILE=$ITA_INSTALL_SCRIPTS_DIR/astroll_answers.txt
ITA_BUILDER_SETTING_FILE=$ITA_INSTALL_SCRIPTS_DIR/astroll_builder_setting.txt
...
if [ "${exec_mode}" == "1" ]; then
    ITA_BUILDER_LOG_FILE=$ITA_INSTALL_SCRIPTS_DIR/log/astroll_gather.log
else
    ITA_BUILDER_LOG_FILE=$ITA_INSTALL_SCRIPTS_DIR/log/astroll_builder.log
fi
...
#check (astroll_builder_setting.txt)
...
if [ "$ACTION" == "Install" ]; then
    if [ "$exec_mode" == 2 ]; then
        log "==========[START astroll BUILDER OFFLINE]=========="
        END_MESSAGE="==========[END astroll BUILDER OFFLINE]=========="
        
    elif [ "$exec_mode" == 3 ]; then
        log "==========[START astroll BUILDER ONLINE]=========="
        END_MESSAGE="==========[END astroll BUILDER ONLINE]=========="
    fi
    
    make_ita
elif [ "$ACTION" == "Download" ]; then
    log "==========[START astroll GATHER LIBRARY]=========="
    END_MESSAGE="==========[END astroll GATHER LIBRARY]=========="
    download
else
