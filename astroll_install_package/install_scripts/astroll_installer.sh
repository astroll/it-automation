...
func_exit_and_delete_file() {
    if ! test -e /tmp/astroll_answers.txt ; then
        rm -rf /tmp/astroll_answers.txt
    fi
    exit
}
...
LOG_FILE="$LOG_DIR/astroll_installer.log"
ANSWER_FILE="$BASE_DIR/astroll_answers.txt"
...
COPY_ANSWER_FILE="/tmp/astroll_answers.txt"
INSTALL_MODE=''
ITA_DIRECTORY=''
ITA_LANGUAGE=''
ITA_OS=''
