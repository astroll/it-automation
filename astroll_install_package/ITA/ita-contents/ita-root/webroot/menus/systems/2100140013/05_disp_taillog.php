...
//
    ...
    $strColIdOfOfRefreshInt      = 'ANSTWR_REFRESH_INTERVAL';
    //----オーケストレータ別の設定記述----
    ...
    try{
        ...
        require_once("{$root_dir_path}/libs/backyardlibs/ansibletower_driver/AnsibleTowerCommonLib.php"); // addPadding
        ...
        if( $objQuery->getStatus()===false ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-501",array($strIfTableIdForSelect)) );
        }
        ...
        if (!$r){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-502",array($strIfTableIdForSelect)) );
        }
        ...
        if( $num_of_rows != 1 ){
            ...
            // 例外処理へ
            throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-503",array($strIfTableIdForSelect)) );
        }
        ...
        // tail対象をtail(読み込み)
        if (isset($_GET['load'])){
            // AnsibleTowerインタフェース情報をローカル変数に格納
            ...
            if( !array_key_exists( "execution_no", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-504") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($execution_no) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-505") );
                }
                unset($objIntNumVali);
            }
            ...
            if( !array_key_exists( "prg_recorder", $_GET ) ){
                ...
                // 例外処理へ
                throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-506") );
            }
            else{
                ...
                if( $objIntNumVali->isValid($prg_record_file_id) === false ){
                    ...
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-507") );
                }
                unset($objIntNumVali);

                if( array_key_exists($prg_record_file_id,$prg_recorder_array)===false ){
                    // 例外処理へ
                    throw new Exception( $objMTS->getSomeMessage("ITAANSTWRH-ERR-510") );
                }
            ...
            // tail対象のファイルパスを作成                               //
            ////////////////////////////////////////////////////////////////
            $if_dir                         = $drs_root_path_from_itaweb . "/" . addPadding($execution_no);
            ...
            if(!file_exists($prg_record_file_name_fullpath) || is_dir($prg_record_file_name_fullpath)){
                ...
                throw new Exception( '<div id="tail_show" style="display:none;"></div>'.$objMTS->getSomeMessage("ITAANSTWRH-ERR-511") );
            }

            // ログファイルの内容を展開
            $lockErrMsg = '<div id="tail_show" style="display:none;"></div>' . $objMTS->getSomeMessage("ITAANSTWRH-ERR-511") . "TODO セマフォロック失敗";

            // ファイル入出力排他処理
            $semaphore = getSemaphoreKey(); // 確認君内の処理とロックを取り合う(ExecuteDirector#createAnsibleLogs())
            try {
                $tryCount = 0;
                while(true) {
                    ...
                    $tryCount++;
                    if($tryCount > 50) {
                        throw new Exception($lockErrMsg . " file lock fail.");
                    }
                }

                // LOCK用ファイルをロック中にやりたい処理 => 実ファイル読み込み
        ...
        <body>
            <div id="iframeInDispTail">
                <table border="0">
                    <tr>
                        <!--//フィルタ-->
                        <td style="padding-right:10px">{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1506010")}：</td>
                        <td>
                            <input onkeydown="pre_filter(event.keyCode)" type="text" id="filter_string" name="filter_string" size="20" maxlength="256">
                        </td>
                        <td style="padding-right:10px">
                            <!--//該当行のみ表示-->
                            <input type="checkbox" id="match_line_only" name="match_line_only" value="on" onClick="pre_filter('13')" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1506020")}
                        </td>
                    </tr>
                </table>
                <br>
