...
//
    ...
    $strExeTableIdForSelect    = 'C_ANSTWR_EXE_INS_MNG';
    
    $strIfTableIdForSelect     = 'B_ANSTWR_IF_INFO';
    
    $strColIdOfDRSRPathFromWebSv = 'ANSTWR_STORAGE_PATH_ITA';
    $strColIdOfTailLine          = 'ANSTWR_TAILLOG_LINES';
    $strColIdOfRefreshInt        = 'ANSTWR_REFRESH_INTERVAL';
    ...
    try{
        ...
        if( !array_key_exists( "execution_no", $_GET ) ){
            // アクセスログ出力(クエリ無し警告)
            web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-103010",__FILE__.':'.__LINE__) );
        }
        ...
        if( array_key_exists( "execution_no", $_GET ) === true ){
            ...
            if( $objIntNumVali->isValid($execution_no) === false ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-103020",__FILE__.':'.__LINE__) );
            }
        }

        $exec_log_caption  = $objMTS->getSomeMessage("ITAANSTWRH-MNU-2000000");
        ...
        if( isset($execution_no) ){
            ...
            if($objQuery->getStatus()===false){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000100")) );
            ...
            if (!$r){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000200")) );
            ...
            if( $num_rows != 1 ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000300")) );
            ...
            if( $objQuery->getStatus()===false ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000600")) );
            ...
            if (!$r){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000700")) );
            ...
            if( $num_of_rows != 1 ){
                // アクセスログ出力(想定外エラー)
                web_log( $objMTS->getSomeMessage("ITAANSTWRH-ERR-404",array(__FILE__,__LINE__,"00000800")) );
    ...
    $aryImportFilePath[] = $objMTS->getTemplateFilePath("ITAANSTWRC","STD","_js");
...
EOD;
    ...
    $strPageExplainBody = $objMTS->getSomeMessage("ITAANSTWRH-MNU-1505090");
...
print <<< EOD
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--説明-->
                <td><div onClick=location.href="javascript:show('SetsumeiMidashi','SetsumeiNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505020")}</div></td>
                <td>
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--対象作業-->
                <td><div onClick=location.href="javascript:show('ExecutionMidashi','ExecutionNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505030")}</div></td>
                <td>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--予約取消-->
                            <td><div onClick=location.href="javascript:show('Booking_Cancel_Midashi','Booking_Cancel_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505040")}</div></td>
                            <td>
                ...
                <div id="Booking_Cancel_Nakami" style="display:block" class="text">
                    <div style="margin-left:10px">
                        <!--予約取消-->
                        <input type="button" id="book_cancel_button" class="updatebutton" value="{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505050")}" onClick=location.href="javascript:book_cancel();" >
                    </div>
                </div>
...
print <<< EOD
                ...
                <h2>
                    <table width="100%">
                        <tr>
                            <!--進行状況-->
                            <td><div onClick=location.href="javascript:show('Monitor_{$loop_counter}_Midashi','Monitor_{$loop_counter}_Nakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505060")}({$prg_recorder['PRG_RCDR_NAME']})</div></td>
                            <td>
...
print <<< EOD
    ...
    <h2>
        <table width="100%">
            <tr>
                <!--緊急停止-->
                <td><div onClick=location.href="javascript:show('ScramMidashi','ScramNakami');" class="midashi_class" >{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505070")}</div></td>
                <td>
    ...
    <div id="ScramNakami" style="display:block" class="text">
        <div style="margin-left:10px">
            <div id="scram_area">
            <!--緊急停止-->
            <input type="button" id="scrumTryExecute" class="updatebutton scrumSubmitElement tryExecute" value="{$objMTS->getSomeMessage("ITAANSTWRH-MNU-1505080")}" onClick=location.href="javascript:scrum_execution();" disabled >
            </div>
        </div>
    </div>
