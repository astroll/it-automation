...
//   false:  異常
////////////////////////////////////////////////////////////////////////////////
function getCopyMasterData( $in_objMTS,
                            $in_objDBCA,
                            $in_cpf_var_name,
                           &$in_cpf_key,
                           &$in_cpf_file_name,
    ...
    if($objQuery->getStatus()===false){
        $in_errmsg = $in_objMTS->getSomeMessage("ITAANSTWRH-ERR-56100",array(basename(__FILE__),__LINE__));
...
//   false:  異常
////////////////////////////////////////////////////////////////////////////////
function chkCPFVarsMasterReg( $in_objMTS, $in_objDBCA, &$ina_cpf_vars_list, &$in_errmsg, &$in_errdetailmsg){
    $boolRet   = true;
    $in_errmsg = "";
    $in_errdetailmsg = "";
    $fatal_error = false;
        ...
        foreach( $tgt_file_list as $tgt_file => $line_no_list ){
            foreach( $line_no_list as $line_no => $cpf_var_name_list ){
                foreach( $cpf_var_name_list as $cpf_var_name => $dummy ){
                    ...
                    $db_errmsg = "";
                    $ret = getCopyMasterData($in_objMTS, $in_objDBCA, $cpf_var_name, $cpf_key, $cpf_file_name, $db_errmsg, $in_errdetailmsg);
                    if( $ret == false ){
                        // DBエラーを優先表示
                        $in_errmsg = $db_errmsg;
                        $boolRet = false;
                    ...
                    if( $cpf_key == "" ){
                        ...
                        $in_errmsg = $in_errmsg . $in_objMTS->getSomeMessage("ITAANSTWRH-ERR-90090",
                                                            array($tgt_file, $line_no, $cpf_var_name));
                        $boolRet = false;
                        continue;
                    }
                    else{
                        ...
                        if($cpf_file_name == "" ){
                            ...
                            $in_errmsg = $in_errmsg . $in_objMTS->getSomeMessage("ITAANSTWRH-ERR-90091",
                                                            array($tgt_file, $line_no, $cpf_var_name));
                            $boolRet = false;
                            continue;
                        }
