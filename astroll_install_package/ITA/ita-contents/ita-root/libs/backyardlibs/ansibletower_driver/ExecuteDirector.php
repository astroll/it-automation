...
$rest_api_command = $root_dir_path . "/libs/backyardlibs/ansibletower_driver/restapi_command/";
...
class ExecuteDirector {
    ...
    function __construct($restApiCaller, $logger, $dbAccess, $exec_out_dir) {
    ...
    function build($exeInsRow, $ifInfoRow) {
        ...
        $ret = $this->prepareProject($execution_no, $ifInfoRow['ANSTWR_STORAGE_PATH_ANSTWR']);
        if($ret == false) {
            // array[40001] = "SCM更新作業に失敗しました。";
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40001");
        ...
        if($ret == false) {
            ...
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40002");
            $this->errorLogOut($errorMessage);
            return -1;
        }
        ...
        if($projectId == -1) {
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40003");
            $this->errorLogOut($errorMessage);
            return -1;
        }
        ...
        foreach($inventoryForEachCredentials as $dummy => $data) {
            ...
            if($credentialId == -1) {
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40004");
                $this->errorLogOut($errorMessage);
                return -1;
            }
            ...
            if($inventoryId == -1) {
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40005");
                $this->errorLogOut($errorMessage);
                return -1;
            }

            // ジョブテンプレート生成
            $jobTemplateId = $this->createEachJobTemplate($execution_no, $loopCount, $projectId, $credentialId, $inventoryId, $exeInsRow['RUN_MODE_ID'], $exeInsRow['I_ANS_PARALLEL_EXE']);
            if($jobTemplateId == -1) {
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40006");
                $this->errorLogOut($errorMessage);
                return -1;
            }
        ...
        if($workflowTplId == -1) {
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40007");
            $this->errorLogOut($errorMessage);
            return -1;
        }
    ...
    private function prepareProject($execution_no, $dataRelayStoragePath) {
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40009");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(count($response_array['responseContents']) === 0
            ...
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40010");
            $this->errorLogOut($errorMessage);
            return false;
        }
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40022");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(count($response_array['responseContents']) === 0
            ...
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40023");
            $this->errorLogOut($errorMessage);
            return false;
        }
        // ホストがあるかのチェックのみ
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40011");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(count($response_array['responseContents']) === 0
            ...
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40012");
            $this->errorLogOut($errorMessage);
            return false;
        }
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40013");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(count($response_array['responseContents']) === 0
            ...
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40014");
            $this->errorLogOut($errorMessage);
            return false;
        }
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40015");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(array_key_exists("id", $response_array['responseContents']) == false) {
            $this->logger->error("No job-template id. (prepare)");
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40016");
            $this->errorLogOut($errorMessage);
            return false;
        }
        ...
        if($response_array['success'] == false) {
            $this->logger->error($response_array['responseContents']['errorMessage']);
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40017");
            $this->errorLogOut($errorMessage);
            return false;
        }
        if(array_key_exists("id", $response_array['responseContents']) == false) {
            $this->logger->error("No job id. (prepare)");
            $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40018");
            $this->errorLogOut($errorMessage);
            return false;
        }
        ...
        while(true) {
            ...
            if($now > $limit) {
                ...
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40019");
                $this->errorLogOut($errorMessage);
                return false;
            }
            $now++;
            ...
            if($response_array['success'] == false) {
                $this->logger->error($response_array['responseContents']['errorMessage']);
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40020");
                $this->errorLogOut($errorMessage);
                return false;
            }
            ...
            if((array_key_exists("status", $responseContents) &&
                ...
                $errorMessage = $this->objMTS->getSomeMessage("ITAANSTWRH-ERR-40021" . $responseContents['status']);
                $this->errorLogOut($errorMessage);
                return false;
            }
    ...
    private function getHostInfo($exeInsRow, &$inventoryForEachCredentials) {
        ...
        $rows = $this->dbAccess->selectRowsUseBind("B_ANSTWR_PHO_LINK", false, $condition);
        ...
        foreach($rows as $ptnOpHostLink) {
            ...
            if(!empty($hostInfo['ANSTWR_INSTANCE_GRP_ITA_MNG_ID'])) {
                $instanceGroup = $this->dbAccess->selectRow("B_ANSTWR_INSTANCE_GROUP", $hostInfo['ANSTWR_INSTANCE_GRP_ITA_MNG_ID']);
    ...
    private function createEachJobTemplate($execution_no, $loopCount, $projectId, $credentialId, $inventoryId, $runMode, $forks) {
        ...
        $param['playbook'] = AnsibleTowerRestApiJobTemplates::LAUNCH_PLAYBOOK_NAME; // 固定
        ...
        $response_array = AnsibleTowerRestApiJobTemplates::post($this->restApiCaller, $param);
    ...
    function monitoring($toProcessRow, $ansibleTowerIfInfo) {
        ...
        // Ansibleログ書き出し
        $ret = $this->createAnsibleLogs($execution_no, $ansibleTowerIfInfo['ANSTWR_STORAGE_PATH_ITA'], $wfJobId);
        if($ret == false) {
            $status = EXCEPTION;
        }
    ...
    function createAnsibleLogs($execution_no, $dataRelayStoragePath, $wfJobId) {
        ...
        $outDirectoryPath       = $dataRelayStoragePath . "/" . addPadding($execution_no) . "/out";
