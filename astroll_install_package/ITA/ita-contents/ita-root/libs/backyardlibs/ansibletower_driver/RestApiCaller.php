...
class RestApiCaller {
    ...
    private $baseURI;
    private $userId;
    private $decryptedPasswd;
    private $decryptedAuthToken;

    private $accessToken;

    function __construct($protocol, $hostName, $portNo, $userId, $encryptedPasswd, $encryptedAuthToken) {
        $this->baseURI = $protocol . "://" . $hostName . ":" . $portNo . self::API_BASE_PATH;
        $this->userId = $userId;
        $this->decryptedPasswd = ky_decrypt($encryptedPasswd);
...
// ------------AnsibleTowe 2.3以前の認証方法
    ...
    function restCall($method, $apiUri, $content = array(), $header = array()) {
        ...
        $this->logger->trace("\n" .
            "+++ REST Start +++\n" .
            "URL: " . $this->baseURI . $apiUri . "\n" .
            "httpContext: \n" . 
            var_export($httpContext, true) . "\n" .
            "++++++++++++++++++\n");
        ...
        if(count($http_response_header) > 0) {
            ...
            $this->logger->debug("method: {$method} url: {$apiUri} status_code: {$status_code}");
