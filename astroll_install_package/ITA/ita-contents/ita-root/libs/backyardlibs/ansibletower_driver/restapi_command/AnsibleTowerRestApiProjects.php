...
class AnsibleTowerRestApiProjects extends AnsibleTowerRestApiBase {
    ...
    const SCM_LOCALPATH_PREFIX = "ita_executions_";
    const IDENTIFIED_NAME_PREFIX = "ita_executions_project_";
    ...
    static function post($RestApiCaller, $param) {
        ...
        if(!empty($param['execution_no'])) {
            $content['name'] = self::createName(self::IDENTIFIED_NAME_PREFIX, $param['execution_no']);
            $content['local_path'] = self::createName(self::SCM_LOCALPATH_PREFIX, $param['execution_no']);
        } else {
            // 必須のためNG返す
            $response_array['success'] = false;
