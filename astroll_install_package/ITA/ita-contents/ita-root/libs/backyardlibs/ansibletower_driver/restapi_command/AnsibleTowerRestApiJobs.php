...
require_once($root_dir_path . "/libs/backyardlibs/ansibletower_driver/restapi_command/AnsibleTowerRestApiJobTemplates.php");

class AnsibleTowerRestApiJobs extends AnsibleTowerRestApiBase {

    const API_PATH  = "jobs/";
    const IDENTIFIED_NAME_PREFIX = "ita_executions_job_";
    const API_SUB_PATH_STDOUT  = "stdout/";
    ...
    static function deleteRelatedCurrnetExecution($RestApiCaller, $execution_no) {
        ...
        // データ絞り込み(親)
        $filteringName = self::createName(AnsibleTowerRestApiJobTemplates::IDENTIFIED_NAME_PREFIX, $execution_no) . "_";
    ...
    static function getStdOut($RestApiCaller, $id) {
        ...
        // REST APIアクセス
        $method = "GET";
        $response_array = $RestApiCaller->restCall($method, self::API_PATH . $id . "/" . self::API_SUB_PATH_STDOUT . "?format=txt", array(), array("Accept: text/plain"));
